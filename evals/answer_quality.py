"""Tier 2 — answer-quality harness (opt-in, needs a real model).
...
The checks protect the core invariant — facts come from the data layer, never
the model — e.g. the quoted price must equal ``best_price``; a cancelled order
must NOT carry a tracking code.
"""
...
def _check_takamine_price(reply: str) -> tuple[bool, str]:
    variants = gt.price_variants(gt.best_price("Takamine GD20"))
    low = _norm(reply)
    ok = any(v.lower() in low for v in variants)
    return ok, f"reply must quote best_price (one of {variants})"
...
ANSWER_CASES: list[AnswerCase] = [
    AnswerCase("price_takamine", "Quanto custa o Takamine GD20?", _check_takamine_price),
